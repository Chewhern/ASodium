...
namespace ASodium
{
    public static class SodiumPublicKeyAuth
    {
        ...
        public static KeyPair GenerateKeyPair()
        {
            ...
            {
                SodiumPublicKeyAuthLibrary.crypto_sign_keypair(PublicKeyIntPtr, SecretKeyIntPtr);
        ...
        public static KeyPair GenerateSeededKeyPair(IntPtr Seed,Boolean ClearKey=false)
        {
            ...
            if (IsZero1 == false && IsZero2 == false)
            {
                ...
                SodiumPublicKeyAuthLibrary.crypto_sign_seed_keypair(PublicKeyIntPtr, SecretKeyIntPtr, Seed);
        ...
        public static Byte[] SignDetached(Byte[] Message, IntPtr SecretKey, Boolean ClearKey = false)
        {
            if (Message == null)
            {
                throw new ArgumentException("Error: Message cannot be null");
        ...
        public static IntPtr ExtractSeedIntPtr(IntPtr SecretKey, Boolean ClearKey = false) 
        {
            ...
            Boolean IsZero = true;
            IntPtr SeedIntPtr = SodiumGuardedHeapAllocation.Sodium_Malloc(ref IsZero, GetSeedBytesLength());

            if (IsZero == false) 
            {
                SodiumGuardedHeapAllocation.Sodium_MProtect_ReadOnly(SecretKey);
                int result = SodiumPublicKeyAuthLibrary.crypto_sign_ed25519_sk_to_seed(SeedIntPtr, SecretKey);
                SodiumGuardedHeapAllocation.Sodium_MProtect_NoAccess(SecretKey);
