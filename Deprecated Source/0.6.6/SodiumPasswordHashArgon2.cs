...
namespace ASodium
{
    public static class SodiumPasswordHashArgon2
    {
        ...
        public static IntPtr Argon2PBKDFCustom(long DerivedKeyLength, IntPtr Password, long PasswordLength, Byte[] Salt, ulong OpsLimit, long MemLimit, Algorithm algorithm = Algorithm.DEFAULT,Boolean ClearKey=false)
        {
            if (DerivedKeyLength != 0)
            {
                if ((DerivedKeyLength >= GetMinPBKDFLength() && DerivedKeyLength <= GetMaxPBKDFLength()) == false)
            ...
            if (IsZero == false) 
            {
                ...
                if (algorithm == Algorithm.ARGON2I)
                {
                    result = SodiumPasswordHashArgon2Library.crypto_pwhash(DerivedKeyIntPtr, DerivedKeyLength, Password, PasswordLength, Salt, OpsLimit, MemLimit, (int)Algorithm.ARGON2I);
                }
                else
                {
                    result = SodiumPasswordHashArgon2Library.crypto_pwhash(DerivedKeyIntPtr, DerivedKeyLength, Password, PasswordLength, Salt, OpsLimit, MemLimit, (int)Algorithm.ARGON2ID);
                }
        ...
        public static IntPtr Argon2PBKDF(long DerivedKeyLength, IntPtr Password, long PasswordLength,Byte[] Salt, Strength strength=Strength.MODERATE, Algorithm algorithm = Algorithm.DEFAULT,Boolean ClearKey=false)
        {
            if (DerivedKeyLength != 0)
            {
                if ((DerivedKeyLength >= GetMinPBKDFLength() && DerivedKeyLength <= GetMaxPBKDFLength()) == false)
        ...
        public static String Argon2HashPassword(IntPtr Password, long PasswordLength,Strength strength = Strength.MODERATE, Boolean ClearKey = false)
        {
            if (Password == IntPtr.Zero)
            {
                throw new ArgumentException("Error: Password cannot be null");
            ...
            SodiumGuardedHeapAllocation.Sodium_MProtect_ReadOnly(Password);
            int result = SodiumPasswordHashArgon2Library.crypto_pwhash_str(HashedPasswordWithParam, Password, PasswordLength, OpsLimit, MemLimit);
            SodiumGuardedHeapAllocation.Sodium_MProtect_NoAccess(Password);
        ...
        public static String Argon2CustomParamHashPassword(IntPtr Password, long PasswordLength,ulong OpsLimit, long MemLimit, Boolean ClearKey = false)
        {
            if (OpsLimit == 0)
            {
                throw new ArgumentException("Error: Ops Limit cannot be 0");
        ...
        public static Boolean VerifyPasswordString(String HashedPasswordWithParamString, IntPtr Password, long PasswordLength,Boolean ClearKey = false)
        {
            if (HashedPasswordWithParamString.LongCount() == 0)
            {
                throw new ArgumentException("Error: Hashed Password With Param String length should not be 0");
            ...
            SodiumGuardedHeapAllocation.Sodium_MProtect_ReadOnly(Password);
            int result = SodiumPasswordHashArgon2Library.crypto_pwhash_str_verify(HashedPasswordWithParamString, Password, PasswordLength);
            SodiumGuardedHeapAllocation.Sodium_MProtect_NoAccess(Password);
