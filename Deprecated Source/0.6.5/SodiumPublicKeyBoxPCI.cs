    ...
    public static class SodiumPublicKeyBoxPCI
    {
        ...
        public static Byte[] CalculateSharedSecret(Byte[] OtherUserPublicKey,Byte[] CurrentUserPrivateKey,Boolean ClearKey=false) 
        {
            ...
            else 
            {
                ...
                {
                    throw new ArgumentException("Error: Other User Public Key must be " + SodiumPublicKeyBox.GetPublicKeyBytesLength() + " bytes in length");
                }
            }
            ...
            {
                if (CurrentUserPrivateKey.Length != SodiumPublicKeyBox.GetSecretKeyBytesLength())
                {
                    throw new ArgumentException("Error: Current User Private Key must be " + SodiumPublicKeyBox.GetPublicKeyBytesLength() + " bytes in length");
                }
            }
        ...
        public static IntPtr CalculateSharedSecret(Byte[] OtherUserPublicKey, IntPtr CurrentUserPrivateKey,Boolean ClearKey=false)
        {
            ...
            if (IsZero == false) 
            {
                ...
                int result = SodiumPublicKeyBoxLibrary.crypto_box_beforenm(SharedSecretIntPtr, OtherUserPublicKey, CurrentUserPrivateKey);
        ...
        public static Byte[] Create(Byte[] Message, Byte[] Nonce, Byte[] SharedSecret,Boolean ClearKey=false)
        {
            ...
            if (Nonce == null || Nonce.Length != SodiumPublicKeyBox.GetNonceBytesLength())
                throw new ArgumentException("Error: Nonce must be " + SodiumPublicKeyBox.GetNonceBytesLength() + " bytes in length");
        ...
        public static Byte[] Open(Byte[] CipherText, Byte[] Nonce, Byte[] SharedSecret, Boolean ClearKey = false)
        {
            ...
            if (CipherText[0] == 0)
            {
                ...
                var trim = true;
                for (var i = 0; i < SodiumPublicKeyBox.GetMACBytesLength() - 1; i++)
                {
                    if (CipherText[i] != 0)
                    {
                        trim = false;
                ...
                if (trim)
                {
                    var temp = new Byte[CipherText.Length - SodiumPublicKeyBox.GetMACBytesLength()];
                    Array.Copy(CipherText, SodiumPublicKeyBox.GetMACBytesLength(), temp, 0, CipherText.Length - SodiumPublicKeyBox.GetMACBytesLength());

                    CipherText = temp;
                }
            }

            Byte[] Message = new Byte[CipherText.Length - SodiumPublicKeyBox.GetMACBytesLength()];
        ...
        public static DetachedBox CreateDetached(Byte[] Message, Byte[] Nonce, Byte[] SharedSecret, Boolean ClearKey = false)
        {
            ...
            Byte[] MAC = new byte[SodiumPublicKeyBox.GetMACBytesLength()];
        ...
        public static byte[] OpenDetached(Byte[] CipherText, Byte[] MAC, Byte[] Nonce, Byte[] SharedSecret,Boolean ClearKey=false)
        {
            ...
            if (MAC == null || MAC.Length != SodiumPublicKeyBox.GetMACBytesLength())
                throw new ArgumentException("Error: MAC must be " + SodiumPublicKeyBox.GetMACBytesLength() + " bytes in length");
