...
namespace ASodium
{
    public static class SodiumPasswordHashScryptSalsa208SHA256
    {
        ...
        public static IntPtr PBKDF2(long DerivedKeyLength, IntPtr Password, long PasswordLength,Byte[] Salt, STRENGTH strength = STRENGTH.INTERACTIVE, Boolean ClearKey = false)
        {
            if (DerivedKeyLength == 0)
            {
                throw new ArgumentException("Error: Derived Key Length must not be 0");
            ...
            Boolean IsZero = true;
            IntPtr DerivedKeyIntPtr = SodiumGuardedHeapAllocation.Sodium_Malloc(ref IsZero, DerivedKeyLength);
            if (IsZero == false) 
            {
                SodiumGuardedHeapAllocation.Sodium_MProtect_ReadOnly(Password);
                int result = SodiumPasswordHashScryptSalsa208SHA256Library.crypto_pwhash_scryptsalsa208sha256(DerivedKeyIntPtr, DerivedKeyLength, Password, PasswordLength, Salt, OpsLimit, MemLimit);
                SodiumGuardedHeapAllocation.Sodium_MProtect_NoAccess(Password);
            ...
            {
                DerivedKeyIntPtr = IntPtr.Zero;
            }

            if (ClearKey == true)
            {
                SodiumGuardedHeapAllocation.Sodium_MProtect_ReadWrite(Password);
                SodiumGuardedHeapAllocation.Sodium_Free(Password);
            }
        ...
        public static Byte[] CustomPBKDF2(long DerivedKeyLength, Byte[] Password, Byte[] Salt, long OpsLimit,long MemLimit,Boolean ClearKey=false)
        {
            ...
            else 
            {
                ...
                {
                    throw new ArgumentException("Error: Ops Limit must between " + GetMinimumOpsLimit() + " and " + GetMaximumOpsLimit());
                }
            }
            ...
            else
            {
                ...
                {
                    throw new ArgumentException("Error: Mem Limit must between " + GetMinimumMemLimit() + " and " + GetMaximumMemLimit());
                }
            }
        ...
        public static IntPtr CustomPBKDF2(long DerivedKeyLength, IntPtr Password, long PasswordLength, Byte[] Salt, long OpsLimit, long MemLimit, Boolean ClearKey = false)
        {
            if (OpsLimit == 0)
            {
                throw new ArgumentException("Error: Ops Limit mustn't be 0");
            ...
            else
            {
                if (DerivedKeyLength < GetMinimumDerivedKeyLength() || DerivedKeyLength > GetMaximumDerivedKeyLength())
                {
                    throw new ArgumentException("Error: Derived Key Length must between " + GetMinimumDerivedKeyLength() + " and " + GetMaximumDerivedKeyLength());
                }
            }
            if (Password == IntPtr.Zero)
            {
            ...
            if (ClearKey == true)
            {
                SodiumGuardedHeapAllocation.Sodium_MProtect_ReadWrite(DerivedKeyIntPtr);
                SodiumGuardedHeapAllocation.Sodium_Free(DerivedKeyIntPtr);
            }
        ...
        public static String ComputePasswordHash(IntPtr Password, long PasswordLength,STRENGTH strength = STRENGTH.INTERACTIVE, Boolean ClearKey = false)
        {
            if (Password == IntPtr.Zero)
            {
                throw new ArgumentException("Error: Password must not be null");
            ...
            SodiumGuardedHeapAllocation.Sodium_MProtect_ReadOnly(Password);
            int result = SodiumPasswordHashScryptSalsa208SHA256Library.crypto_pwhash_scryptsalsa208sha256_str(ComputedPasswordHashWithParams, Password, PasswordLength, OpsLimit, MemLimit);
            SodiumGuardedHeapAllocation.Sodium_MProtect_NoAccess(Password);
        ...
        public static String CustomComputePasswordHash(IntPtr Password, long PasswordLength,long OpsLimit, long MemLimit, Boolean ClearKey = false)
        {
            if (OpsLimit == 0)
            {
                throw new ArgumentException("Error: Ops Limit mustn't be 0");
        ...
        public static Boolean VerifyPassword(String ComputedPasswordHashWithParams, IntPtr Password, long PasswordLength, Boolean ClearKey = false)
        {
            if (ComputedPasswordHashWithParams == null)
            {
                throw new ArgumentException("Error: Computed Password Hash With Params mustn't be null");
            ...
            SodiumGuardedHeapAllocation.Sodium_MProtect_ReadOnly(Password);
            int result = SodiumPasswordHashScryptSalsa208SHA256Library.crypto_pwhash_scryptsalsa208sha256_str_verify(ComputedPasswordHashWithParams, Password, PasswordLength);
            SodiumGuardedHeapAllocation.Sodium_MProtect_NoAccess(Password);
